...
├─ 2026_Expenses.xlsx ← 年度別スプレッドシート
├─ 2027_Expenses.xlsx ← 翌年追加（前年データ保持）
├─ Receipts/
│   ├─ 2026-01/（月別フォルダ）
│   │   └─ receipt_123456.jpg
│   ├─ 2026-02/
...
2. **スプレッドシート作成**: `2026_Expenses.xlsx`（ルート配下に配置）
3. **Receiptsフォルダ作成**: `Receipts/` フォルダ
4. **月別フォルダ作成**: `2026-01/` から `2026-12/` まで一括作成

### 翌年対応挙動

次の年（例: 2027年）になってセットアップボタンを押した場合：

- **前年データ完全保持**: 2026年のファイル・フォルダは一切変更/削除されません
- **新規追加のみ**: 2027年用のファイル・フォルダのみを新規作成
- **独立運用**: 2026年と2027年のデータを並行して使用可能

#### 作成されるもの（2027年分）:
```
Gemini Expense Tracker/
├─ 2026_Expenses.xlsx ✅（既存保持）
├─ 2027_Expenses.xlsx 🆕（新規追加）
├─ Receipts/
...
### Phase C 🎯（進行中）：別表A自動生成 + 税務チェックエンジン
...
2. **アプリ UI の役割（見える・転記対象）**
   - 別表A「第一表」を HTML で完全再現
   - 別表A「第二表」を HTML で完全再現
...
**重要な哲学**
- 別表A は税務署から「完全なフォーマット」を要求される
- Sheets での印刷は NG（フォーマットズレで差し戻し）
- このアプリは「**転記ミスを根絶する装置**」
- 派手さではなく、静かに失敗を潰すツール

#### C1. 別表A自動作成【本丸】
- Google Sheets で勘定科目 → 別表A項目の自動マッピング
- SUMIFS で機械的集計
- **未分類・曖昧データだけ Gemini 3 で分類**
- 別表A フォーマットの自動生成

#### C2. 月次・四半期レポート自動生成
- QUERY / SUMIFS による動的集計
- API で JSON 化して UI 表示
- 売上・経費・利益の概要表示

#### C3. 税務チェックリストエンジン
- ルールベースの自動チェック機能
- 例：「通信費が売上の20%超 → 私用混在の可能性」
- AI ではなく if 文で実装

**Gemini 3 の使い方**
- 通常の取引：Flash で十分
- 未分類・例外データ：Gemini 3 で会計脳を発揮
- 「派手に見せない AI」が一番価値がある
...
この取引は別表Aのどの勘定科目に分類すべきか。
日本の個人事業主の確定申告の実務に基づいて、
最も妥当な科目を1つだけ返せ。
...
月次集計	なし
税務チェック	なし
未分類・例外分類	Gemini 3 ← 唯一のAI核心

**Phase C のゴール**
> 「レシート掘り返す必要ゼロ。シート見ながらe-Tax入力するだけ」

### Phase D 📋（実装予定）：e-Tax ガイド統合
- 電子申告手順の自動生成
- 税務書類のチェック機能
- 提出前の最終確認システム
- 現在の優先度外
...
- モバイルアプリ化
- クラウドネイティブアーキテクチャ
**ポイント：** ここまで来たら 「アプリ」から **「確定申告の相棒」** に昇格する。- 現在の優先度外
...
* Gemini ハッカソンで求められる「推論能力を活かしたUX」の具体例
